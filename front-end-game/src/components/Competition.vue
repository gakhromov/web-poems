            ...
            <div class="col-xl-9 col-lg-9 col-md-9 col-sm-12">
                ...
                <span v-for="(line, index) in poemLines" :key="index">
                    <div v-if="line.player == null" class="row">
                        <div data-aos="fade-down" class="col-3"><p>Компьютер</p></div>
                        <div data-aos="fade-down" class="col-9">{{line.line}}</div>
                    </div>
                    <div v-else-if="typeof line.line === 'string'" class="row">
                        <div data-aos="fade-down" class="col-3"><p>{{username}}</p></div>
                        <div data-aos="fade-down" class="col-9">{{line.line}}</div>
                    </div>
                    <div v-else></div>
                </span>
                <div v-if="poemLines[poemLines.length-2].line == null" class="row" style="margin-bottom: 10px">
                    ...
                    <div data-aos="fade-down" class="col-9"><input v-model="firstLine" type="text" class="form-control"/></div>
                </div>
                <div v-if="poemLines[poemLines.length-1].line == null" class="row" style="margin-bottom: 10px">
                    ...
                    <div data-aos="fade-down" class="col-9"><input v-model="secondLine" type="text" class="form-control"/></div>
                </div>
                <div v-if="gameFinished">
                    <h4 data-aos="fade">Поздравляем, <b>соревнование</b> завершено!</h4>
                    <label data-aos="fade-down">Ваш результат:</label><br>
                    <label data-aos="fade-down" class="result-notice">Количество набранных баллов = <b>{{score}}</b>. <br>Место в лидерборде = <b>{{rank}}</b></label>
                </div>
            </div>
